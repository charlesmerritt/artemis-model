...
- [Claude Code on the web session setup](claude-code-web-environment.md) — what `.claude/hooks/session-start.sh` provisions in the ephemeral container (CPython 3.14 via `uv sync`, `core.hooksPath`), and the egress gotcha behind it: `INSTALL sqlite` cannot reach `extensions.duckdb.org` (HTTP 403), which fails all nine `tests/test_restart_fidelity.py` tests there but not locally. Fixed by staging the PyPI-published extension binary into `~/.duckdb`.
