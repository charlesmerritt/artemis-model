...
| Item | Status |
|---|---|
| Riparian buffers grow freely, are never harvested, and are reported as unique buffer polygons | **Decided and implemented.** Enforced structurally: a riparian unit's trajectory library contains only `no_management`, so no-entry is not a constraint the scheduler could weigh. `sliver_merge.py` preserves `unit_class`. |
...
| Ownership's role | **Resolved: defines the eligible set, not the choice.** `config/management_regimes.yaml`; the scheduler selects within it by simulated annealing |
