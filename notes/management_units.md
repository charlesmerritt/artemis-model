...
- ~~Retain riparian/BMP buffers as their own `unit_class = "riparian"` units instead of erasing them.~~ **Done 2026-08-03.** Buffers are retained with `buffer_class`, managed and riparian units partition the eligible forest (enforced to 0.01 ha), the permanently-excluded area is reported in `area_accounting.csv`, and riparian units are exempt from both the 40 ha split and sliver resolution. See [`methodology-directions.md`](methodology-directions.md) item 2 and [`../docs/config-policy.md`](../docs/config-policy.md).
- The Union County smoke figures below (17,020 candidate polygons) predate that change and are **not comparable** to a current run: buffered acres are now retained as riparian units rather than erased, so both polygon count and total area go up. Re-run before quoting either number.
