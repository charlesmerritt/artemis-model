...
Added `pipeline/s3_management/sketch_management_units.py` as the first production-style script for draft management units. It processes by Florida county so statewide parcels are never loaded all at once; clips parcels, roads, NHD flowlines, NHD waterbodies, and LANDFIRE EVT; builds a forest mask from the EVT VAT; intersects forested parcels; erases water and a small road buffer; **builds the Florida BMP buffer layer but does not apply it** (written as `riparian_buffers.gpkg` for `riparian_overlay.py` to apply after sliver resolution); optionally splits large polygons with a simple 40 ha fishnet; and writes per-county GeoPackage outputs plus CSV summaries.
...
- ~~Retain riparian/BMP buffers as their own `unit_class = "riparian"` units instead of erasing them.~~ **Done 2026-08-03.** Delineation is now three ordered steps: `sketch_management_units` → `sliver_merge` → `riparian_overlay`. The buffer layer is built in step 1 and applied in step 3, so it annotates a settled stand map instead of shaping it. Stands stay contiguous (a stream through a stand makes two stands) and the overlay conserves area exactly. See [`methodology-directions.md`](methodology-directions.md) item 2 and [`../docs/config-policy.md`](../docs/config-policy.md).
- The Union County smoke figures below (17,020 candidate polygons) predate that change and are **not comparable** to a current run. Sketch no longer carves parcels along every stream, so its polygon count drops; the overlay then adds riparian stands back at the end, and total forest area goes up because buffered acres are retained rather than erased. Re-run before quoting either number.
